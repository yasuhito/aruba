...
When /^I run "(.*?)" as child "(.*?)"$/ do |cmd, name|
  run(unescape(cmd), name)
end

When /^I type "(.*?)" to child "(.*?)"$/ do |input, name|
  child_write(unescape(input), name)
end

When /^I send signal "([^\"]*)" to child "([^\"]*)"$/ do |signal, name|
  child_sig(signal, name)
end
...
Then /^I should see "(.*?)" from child "(.*?)"$/ do |partial_output, name|
  combined_output(name).should =~ compile_and_escape(partial_output)
end

Then /^the child "([^\"]*)" should not be running$/ do |child|
  child_running?(child).should == false
end
...
Then /^the exit status of "(.*?)" should be (\d+)$/ do |name, exit_status|
  child_exit_status(name).should == exit_status.to_i
end
